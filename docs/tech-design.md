...
- **M1 — Web thin slice (this design).** Everything in §7–§11, including the ingest-time index (vectors + FTS). Single-tenant, single user. *Definition of done in §13; phase-by-phase plan with agent briefs in the [implementation plan](./implementation-plan.md).*
...
- **M2 — Interesting-things digest.** A **Cloudflare Workflow** (durable steps: query-plan → multi-source fetch across free sources (Reddit/HN/web) → rank into scored "evidence clusters" → synthesize) + AI SDK calls, scheduled via Cron Triggers — the `mvanhorn/last30days-skill` *pattern*, Worker-native. A pipeline, not an autonomous agent.
