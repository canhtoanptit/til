...
- **Expo / React Native.** Best *native* mobile feel and web via `react-native-web`. Rejected because: React-DOM chat UI libraries (AI Elements / assistant-ui) can't be reused — the chat UI would be rebuilt in RN; desktop is the weak spot (community Electron wrappers); larger divergence from our all-TS/DOM stack.
...
- **PWA instead of Tauri (M4).** Installable on desktop and mobile home screens with zero store friction — a serious cheapener for M4. Kept as the *first* M4 step; Tauri remains the path when native-shell capabilities (system tray, share targets, offline SQLite) are wanted.
