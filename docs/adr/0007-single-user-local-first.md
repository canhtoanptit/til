...
- **`PUT /api/settings` is full-replace, with one narrow exemption** (amended 2026-08-03 after P5 usability testing): `apiKey` may be omitted to keep the stored key **only when `provider`, `cf_account_id`, and `cf_gateway_id` are all unchanged**. Any change to those routing fields requires re-supplying the key. This preserves the property the rule exists for — a caller who holds the app token but cannot _read_ the key (it is always masked) can never cause the real key to be sent to a gateway they control — while allowing routine edits (model, gateway token) without re-typing the key. `cf_aig_token` is exempt from the guard because it authenticates _to_ the user's own gateway and cannot redirect the key.
- **WebSocket auth (added 2026-08-08 for the M3 chat agent).** Browsers cannot set headers on a WS handshake, and two header-based carriers were tried and rejected on evidence: `Sec-WebSocket-Protocol` breaks because the client-side `ws` implementation errors when the server doesn't echo an offered subprotocol, and `@cloudflare/vite-plugin` strips that header from forwarded responses in dev. So `POST /api/chat/ticket` (bearer-authed) mints a **short-lived signed ticket** — `<expiresAt>.<base64url(HMAC-SHA256(APP_TOKEN, "til-chat-ticket:" + expiresAt))>`, 60 s TTL, stateless, constant-time verified, reusable within its window (single-use would break the socket library's auto-reconnect). Tickets are accepted **only** on a WebSocket upgrade under `/api/chat/` — never for REST — so a URL is never a credential for anything else. Accepted tradeoff: the ticket appears in access logs for up to a minute; `APP_TOKEN` never does.
