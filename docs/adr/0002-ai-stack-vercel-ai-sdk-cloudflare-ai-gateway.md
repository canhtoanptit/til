...
- **History:** v1 chose Pi (`pi-ai` + `pi-agent-core` + `pi-web-ui`). Superseded in-place after review — see *Why the reversal* below.
...
- **Cloudflare Agents SDK as the whole AI layer.** It is a serving/state layer, not a model-access layer. Adopted *selectively* for M3 chat serving, not as the stack.
