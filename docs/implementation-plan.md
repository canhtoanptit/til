...
- **P27 — Email (deployed instance only). DEFERRED by owner 2026-08-13.** Email Routing requires a custom domain on the Cloudflare account (workers.dev can't route email); none is set up, and the owner chose to defer rather than acquire one now. Contract stands as sketched for whenever a domain exists: (a) weekly digest delivery via the **`send_email` binding** to the owner's Email-Routing-verified address — keyless, no third-party ESP; (b) **email-in capture**: Email Routing → Email Worker → extract links → ingest pipeline. Both verified against current Email Workers docs at dispatch. Note for that dispatch: the destination address is deployment-specific info — it must live in D1 settings or a deploy-time local config edit, never in the tracked wrangler.jsonc.

Out of scope permanently (unchanged non-goals): multi-user/sharing, native mobile UI, beating paywalls.

### P28 — hardening & polish wave (planned 2026-08-13; runs before M4)

Pays down the debt every FEAT-wave agent flagged, before P15/P16 build further on the client. Three slices, dispatchable in parallel (a ∥ b; c after a):

- **P28a — client test infrastructure.** Add jsdom + @testing-library/react (dev-deps only) and the `@/` alias to the web vitest config; first render tests for the repeatedly-flagged untested surfaces: ReviewPage card flow, ⌘K palette, export card download flow, feedback thumbs, DigestSourcesCard. This is the recurring "covered by typecheck/build only" gap, closed.
- **P28b — repo hygiene.** Fix the literal NUL byte in `digest.ts` (restores text diffs/merges for the digest core); repo-wide `pnpm format` as its own isolated commit (~85 files fail `format:check` at baseline); decide the `migrations/meta/` journal question (delete it or document the hand-written policy in the migrations dir); sync `docs/tech-design.md` (entries endpoint now has `filter`/`tag`; export §8.1 exists) and README feature list.
- **P28c — small owed features.** "Run monthly report" button on DigestListPage; `enrolledCount` on the review queue response for a true first-run state; `GET /api/feedback` reader (unlocks persisted thumb state cheaply — swap `NO_VOTES` for a seeded map).

Explicitly deferred, not forgotten: P27 email (needs a domain), P14.2 render-loop bug (owner: low), InnerTube YouTube transcript path (P25's proposal — revisit if video capture matters), evals v2 promptfoo judged layer (ADR-0011 staged decision), digest-ranking eval dataset (P21's noted gap), `VectorStore.getVectors` batch read (only if digest frequency rises).
...
| 2026-08-19 | P28a  | phase-standard subagent (worktree)                                                                              | **Done, DoD verified** (1353→1395 tests: web 671→713, of which client render tests 0→97 across 5 surfaces — ReviewPage, CommandPalette, ExportBackupCard, chat feedback thumbs + ChatPage wiring, DigestSourcesCard). Harness: vitest `test.projects` (verified against installed 4.1.10 types — `workspace`/`environmentMatchGlobs` are gone in 4.x), worker project stays node, client project gets **happy-dom** + `@/` alias + RTL cleanup setup. **happy-dom over jsdom, decided by probe**: jsdom@30's Node range is incompatible with the repo's `engines.node: >=20`, jsdom@29 lacks the ResizeObserver cmdk needs, and happy-dom starts 2.7× faster. **Real timers + `waitFor` over fake clocks**: React 19 flushes effects through a scheduler `vi.advanceTimersByTime` does not drive (all three fake-timer recipes hung); debounce asserted via not-called-synchronously-then-eventually-once; client project ran 5× consecutively with zero flakes. Zero component changes, zero bugs found, zero testability refactors — everything was already exported. Client bundle **sha1-identical** to base. Test patterns documented in a ~50-line comment in `ReviewPage.test.tsx`; key gotchas recorded: **stateful fakes not fixed responses** (mutations invalidate queries — a mocked list refetch un-does the test), partial-mock `../api` (`ApiError` is a real class components branch on), never render `ui/sonner`'s Toaster outside ThemeProvider. Lockfile kept clean (jsdom spike leftovers purged by restoring base lockfile).                                                                                                              | @testing-library/react 16.3.2, @testing-library/dom 10.4.1, @testing-library/user-event 14.6.5, happy-dom 20.11.2 (exact-pinned devDeps)                                                                                                       |
| 2026-08-19 | P28b  | phase-standard subagent (worktree)                                                                              | **Done, DoD verified** (1353 tests unchanged; typecheck/lint clean; worker bundle byte-identical; `format:check` passes on its branch). **NUL byte in `digest.ts` fixed — and the brief's diagnosis corrected**: git sniffs only the first 8000 bytes for NUL and this one sat at 12054, so git was already diffing the file as text; the real, silent symptom was **grep-class tooling flagging binary on any NUL** — `grep -c sourceName digest.ts` returned no-matches/exit-1 before the fix and `5` after, meaning every agent that grepped this file was told its symbols didn't exist. Replaced with a ` ` escape (identical runtime string). **Trap recorded: the Write tool turns a typed ` ` into a raw NUL and Read renders NUL as a space — verify with node/xxd, never Read.** **`migrations/meta/` kept frozen, not deleted** — drizzle-kit is live (devDep 0.31.10 + drizzle.config.ts + `db:generate`); new `packages/db/migrations/README.md` documents the hand-written policy, next-free 0012 (reserved P27), and the undocumented footgun that `db:generate` today would emit one giant migration re-creating 0004–0011 (journal stops at 0003). `meta/` added to `.prettierignore` in the substantive commit so it survives dropping the format commit. Doc sync: tech-design API table current through FEAT2 (§7 no longer calls shipped tables "planned"); README status/features/Coming-next rewritten (+ADR count 9→12, evals in structure, deploy.md link re-homed). Repo-wide format kept as a separate droppable commit per orchestrator plan (regenerated at integration after P28a).                                          | no new deps                                                                                                                                                                                                                                   |
| 2026-08-13 | —     | orchestrator (Fable 5)                                                                                          | **M-FEAT2 wave complete on `feat/m-feat2`** (P23→P24→P26→P25 integrated in landing order, 967→1353 tests, typecheck/lint clean at every merge; pending owner review → merge → migrations 0009/0010/0011 remote → deploy). **P27 deferred by owner** (no custom domain for Email Routing; contract preserved in §P23‥P27). **P28 hardening & polish wave planned** (client test infra ∥ repo hygiene; small owed features after) — see new §P28. Execution order ahead: release FEAT2 → P28 → P15 PWA → P16 Tauri.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | —                                                                                                                                                                                                                                             |
| 2026-08-13 | P25   | phase-standard subagent (worktree)                                                                              | **Done, DoD verified** (+238 tests on its base: core 398→495, web 452→586, db 36→43). Content types: migration `0010_content_type.sql` (default `'article'`, no CHECK — SQLite can't ADD one without a table rebuild; enforced in code via `normalizeContentType` per `status` precedent). Detection pure in `@til/core/content-type.ts` (110 tests): URL-phase guess **refined at fetch time from the response header** (a `.pdf` URL serving HTML demotes to article; extensionless PDF promotes), content type settled BEFORE fallible work so a failed PDF never wears an article badge. **PDF via `env.AI.toMarkdown` — facts verified against docs (binding page 2026-07-13) + installed types**: `{name, blob}` in, failures **in-band** (`format:"error"`), response field is `mimeType` (docs page's `mimetype` is the typo — repo interface corrected), **no OCR** (scanned PDFs get their own message), bills no neurons, 5MB fetch cap retained in front. Local stack: readable failure ("requires the cloud stack"). Extractor gains **optional `documentToMarkdown`** — capability-absence is the signal, which also correctly covers cloud-with-missing-binding. **YouTube keyless transcripts, probed live 2026-08-12**: JSON.parse of `ytInitialPlayerResponse` (regexing the `&`-escaped baseUrl yields an unfetchable URL — asserted), format-sniffing parser (json3/srv1/srv3), named degradation for **bot-check (`LOGIN_REQUIRED` — expect this to be the COMMON case from Worker IPs)** and **proof-of-origin caption URLs (`exp=xpe` → empty 200 body, detected without spending the fetch)**; every failure is one ExtractionError carrying the experimental sentence; route always 201s. Real bug found by its own test: json3 cue concatenation lacked separators. Judgement call applied: `POST /api/entries` response also gains `contentType` (optimistic pending card badged without a second round-trip). **Deviation proposed not applied: the working keyless path today is the InnerTube `ANDROID_VR`/`IOS` player POST (no `exp=xpe` on its caption URLs, verified live) — a follow-up phase; treat "no transcript" as a normal user-visible state.** | @cloudflare/workers-types 5.20260801.1; YouTube shapes probed live 2026-08-12                                                                                                                                                                 |
| 2026-08-13 | P26   | phase-standard subagent (worktree)                                                                              | **Done, DoD verified** (+69 tests on its base: core 398→411, db 30→33, web 411→464). Monthly reading report: migration `0011_digest_kind.sql` (`digests.kind` default `'weekly'`); second cron `0 9 1 * *` routed on `controller.cron` (**contract verified against current CF scheduled-handler docs at dispatch: verbatim string equality**, so a test reads `wrangler.jsonc`, extracts the cron strings and asserts they equal the routing constants — a stray space now fails a test instead of silently never firing); report rides the existing Workflow via an optional `kind` on `DigestRunParams` (one binding, one retry budget; an instance mid-flight across the deploy replays as the weekly run it started as). Input = the owner's saves: reuses the chat `stats()` aggregations **with the clock pinned to `plan.runAt`** (frozen-window property preserved, tested); prompt pool = ready entries only while counts cover all statuses; same synthesis output schema so `parseSynthesis` makes a report **structurally incapable of citing an unsaved URL**. Empty month → failed-with-reason (a `skipped` status would be relabeled by the stale-pending sweep; the reason distinguishes "nothing saved" from "nothing finished processing"; LLM never called). `POST /api/digests/run` gains strict optional `kind`; kind Badge on list+detail (primary-tinted outline — the hue-stable tokens are spoken for, and chart tokens aren't hue-stable across themes); score chip + "matches your reading" suppressed on reports. `SynthesisInput.score` widened to optional (absent renders "n/a", can never read as "scored zero"). One existing assertion strengthened, none weakened. Gotcha recorded: `persist` now takes an `ItemResolver`; a third digest flavour's resolver must return the same 12 keys or the chunk size shifts. Follow-up owed: no UI button starts a report (API + cron only).                                | wrangler 4.118.0                                                                                                                                                                                                                              |
