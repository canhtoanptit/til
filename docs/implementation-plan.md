...
8. **Model policy (updated 2026-08-09):** implementation agents run on **Opus at max effort** (owner's call: "opus-4-8 or opus 5, effort max"). Dispatch with `model: "opus"` — the tier resolves to the newest Opus in the harness (Opus 5 at time of writing) — and effort inherits the session's `max`. The orchestrator (Fable 5) **plans, dispatches, verifies DoD, and applies small patches only — it does not implement phases itself.** Downshifting a trivial task requires owner approval.
...
#### P14.2 — patch: chat render loop (OPEN — brief parked after an interrupted dispatch)
